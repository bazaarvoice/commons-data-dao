...
public class JSONSchemaMarshaller extends AbstractJSONMarshaller<JSONSchema> {
    ...
    protected JSONArray convertSchemaTypesToJSONValues(Collection<JSONSchemaType<?>> types) {
        List<Object> typeValueList = Lists.newArrayList();
        for (final JSONSchemaType<?> type : types) {
            if (type instanceof JSONSchemaUnionType) {
                ...
                typeValueList.addAll(toJSONList(unionType.getSchemas(), false));
            } else {
                typeValueList.add(convertSchemaTypeToJSONTypeValue(type));
            }
        }
        return new JSONArray(typeValueList);
    }
